...
describe OpenFga::ApiClient do
  context 'initialization' do
    context 'URL stuff' do
      context 'host' do
        it 'removes http from host' do
          OpenFga.configure { |c| c.host = 'http://example.com' }
          expect(OpenFga::Configuration.default.host).to eq('example.com')
        end

        it 'removes https from host' do
          OpenFga.configure { |c| c.host = 'https://wookiee.com' }
          expect(OpenFga::ApiClient.default.config.host).to eq('wookiee.com')
        end

        it 'removes trailing path from host' do
          OpenFga.configure { |c| c.host = 'hobo.com/v4' }
          expect(OpenFga::Configuration.default.host).to eq('hobo.com')
        end
      end
  ...
  describe 'proxy in #build_connection' do
    let(:config) { OpenFga::Configuration.new }
    let(:api_client) { OpenFga::ApiClient.new(config) }
    let(:proxy_uri) { URI('http://example.org:8080') }

    it 'defaults to nil' do
      expect(OpenFga::Configuration.default.proxy).to be_nil
  ...
  describe '#build_collection_param' do
    let(:param) { ['aa', 'bb', 'cc'] }
    let(:api_client) { OpenFga::ApiClient.new }
